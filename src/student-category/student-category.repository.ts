...
export class StudentCategoryRepository extends Repository<StudentCategory> {
    ...
    async addCategory(addStudentCategoryDto: AddStudentCategoryDto): Promise<StudentCategory> {
        const {
            name,
            description,
        ...
        try {
            ...
            this.logger.error(`Failed to create a task for ${JSON.stringify(category)}`);
        }
    }
